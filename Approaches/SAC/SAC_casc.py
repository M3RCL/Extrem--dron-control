...
class SAC(object):
    ...
    def save(self, filename):
        ...
        print("loaded Training steps:",self.train_steps)
...
if __name__ == "__main__":
    ...
    curriculum_episodes = 500  # Over these many episodes, curriculum_factor goes from 0 to 1

    if os.path.exists("drone_SAC_C_last.pth"):
        agent.load("drone_SAC_C_last.pth")
        print("FineTuning!!")
    try:
        for episode in range(episodes):
            # Update curriculum factor based on episode number.
            curriculum_factor = min(1.0, episode / curriculum_episodes+0.5)
            ...
            for t in range(episode_timesteps):
                ...
                if len(replay_buffer.storage) > batch_size:
                    train_results = agent.train(replay_buffer, batch_size)
                    #print(train_results)  # Should show loss values decreasing over time
                if done:
                    break
            print(f"Episode {episode} reward: {episode_reward}, Curriculum factor: {curriculum_factor}")
                    
            if episode % 50 == 0:
                agent.save(f"drone_SAC_C_last.pth")
    except KeyboardInterrupt:
        print("\nTraining interrupted. Saving model...")
        agent.save("interrupted_drone_SAC_C.pth")
